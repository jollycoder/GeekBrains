...
function ChessBoard(parentId) {
    ...
    this._rowsNumber = this._cellsNumber = 8;               // защищённые свойства, постоянные для шахматной доски
    this._letters = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'];
    this._cellsInfo = {};   // коллекция для записи информации о клетках
    Table.call(this, this._rowsNumber, this._cellsNumber);
}
...
ChessBoard.prototype.setPosition = function (jsonFile) {
    var self = this;
    var timeout = 5000, i = 0, timerId;
    var piecesData;
    var pieces = ['K', 'Q', 'R', 'B', 'N', 'p'];
    var ci = this._cellsInfo;

    if (jsonFile) {
        getPositionFromJson(jsonFile);
        timerId = setInterval(ifDataThenSetPosition, 100);
    }
    else {
        getStartPosition();
        ifDataThenSetPosition();
    }

    function ifDataThenSetPosition()  {
        if (timerId)  {
            if (i >= timeout/100)  {
        ...
        validateData() && clearBoard() && setPiecesFromData();
    }

    function getPositionFromJson(url) {
    ...
    function setPiecesFromData() {
        for (var color in piecesData) {
            for (var piece in piecesData[color]) {
                piecesData[color][piece].forEach(function (coord) {
